const SkillCard = ({ heading = ' ', skills = [], hcolor = 'var(--clr-accent)' }) => {
    return (
            ...
            <ul className='skill-list'>
                {skills.map((skill, index) => (
                    <li key={index} className='skill-list-li'>
                        {skill.icon && (
                            <img
                                src={skill.icon.startsWith('https://') ? skill.icon : `https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/${skill.icon}/${skill.icon}-original.svg`}
                                alt={skill.name}
                                width="16px"
                                height="16px"
                            />
